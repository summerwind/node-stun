...
Packet.decode = function decode(buffer) {
    ...
    var attrs = Packet._decodeAttributes(buffer_attrs, header.tid);
...
Packet._decodeAttributes = function _decodeAttributes(buffer, tid) {
    var attrs = {};
    var offset = 0;
    ...
    while (offset < buffer.length) {
        ...
        case Packet.ATTR.XOR_MAPPED_ADDRESS:
            value = this._decodeXORMappedAddress(value, tid);
            break;
...
// Decode XOR_MAPPED_ADDRESS value
Packet._decodeXORMappedAddress = function _decodeXORMappedAddress(buffer, tid) {
    var family = (buffer.readUInt16BE(0) === 0x02) ? 6 : 4;
    var xor_mapped = buffer.readUInt32BE(4);
    var address;
    if (family === 4) {
        address = ip.fromLong((xor_mapped ^ this.MAGIC_KEY));
    } else {
        address = (xor_mapped ^ parseInt((this.MAGIC_KEY.toString(16) + tid.toString(16)), 16));
        var bit_address = address.toString(2);
    }

    return {
        family:  family,
        port:    (buffer.readUInt16BE(2) ^ 0x2112),
        address: address
    };
};
