    ...
    while (offset < buffer.length) {
        ...
        switch (type) {
            case Packet.ATTR.MAPPED_ADDRESS:
                value = this._decodeMappedAddress(value, buffer_header);
            break;
            case Packet.ATTR.XOR_MAPPED_ADDRESS:
                value = this._decodeXorMappedAddress(value, buffer_header);
            break;    
            case Packet.ATTR_ERROR_CODE:
                value = this._decodeErrorCode(value, buffer_header);
            break;
            case Packet.UNKNOWN_ATTRIBUTES:
                value = this._decodeUnknownAttributes(value, buffer_header);
            break;
        }
...
Packet._decodeXorMappedAddress = function _decodeXorMappedAddress(buffer, buffer_header) {
    ...
    var keyBuf = buffer_header.slice(4, 8);  // BE
    var tidBuf = buffer_header.slice(8, 20); // BE

    if (keyBuf.readUInt32BE(0) != Packet.MAGIC_KEY) {
        throw new Error('Invalid packet.');
    }
       
    var xorPortBuf = buffer.slice(2, 4);                     // LE
    var xorAddrBuf = buffer.slice(4, family === 4 ? 8 : 20); // LE

    var portBuf = xor(xorPortBuf, keyBuf.slice(0, 2));
    var addrBuf = xor(xorAddrBuf, family === 4 ? keyBuf : Buffer.concat(keyBuf, tidBuf))

    return {
        family:  family,
        port:    portBuf.readUInt16BE(0),
        address: ip.toString(addrBuf, 0, family)
    };
};
